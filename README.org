#+title: bisos.PyCS:  Python Command-Services Framework.
...
*Spread Planted Command Services (=.spcs=)* --- a PyCS pattern for
planting identical file content at many filesystem locations and
letting each planted instance be characterised by its directory. When
composed with =bisos.fileObj='s tree walker, =.spcs= becomes a
general-purpose framework for walkable trees (=bisos.dockerProc= over
=bro_dockerfiles= is the first end-to-end consumer; future consumers
include =bisos.lcnt=, static-web publication, and others). See
[[file:spcs/README.org][spcs/README.org]] for architecture, figure, and
